...
import MobileSidebar from '@/components/hub/MobileSidebar';
...
import NewsletterSection from '@/components/hub/NewsletterSection';
import LanguageSwitcher from '@/components/hub/LanguageSwitcher';
...
export default function PortfolioPage() {
  ...
  return (
    <main className="relative z-10 bg-[#05070f] text-[#e2e8ff] min-h-screen selection:bg-indigo-500/30">
      ...
      <section className="relative pt-16 pb-20 md:pt-20 md:pb-24 px-4 text-center overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/30 rounded-full px-4 py-1.5 mb-6 md:mb-8">
          <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_8px_#6366f1]" />
          <span className="font-display font-semibold text-[10px] uppercase tracking-widest text-indigo-300">{t.live} — 2026</span>
        </motion.div>

        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="font-display font-extrabold text-3xl sm:text-4xl md:text-6xl lg:text-7xl text-white leading-[1.1] tracking-tight mb-6">
          <span className="bg-gradient-to-r from-indigo-500 via-cyan-400 to-indigo-500 bg-clip-text text-transparent">{t.heroTitle}</span>
        </motion.h1>

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="max-w-lg mx-auto text-[#6b7299] text-sm sm:text-base md:text-lg leading-relaxed mb-4">{t.heroSubtitle}</motion.p>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="max-w-2xl mx-auto text-[#a1a7c5] text-xs sm:text-sm md:text-base leading-relaxed mb-10 md:mb-12">{t.heroDescription}</motion.p>
      ...
      {activeCategory === 'all' && (
        <section className="max-w-7xl mx-auto px-4 pt-12"><StatsOverview /></section>
      )}

      <div className="flex flex-col items-center gap-8 my-16 px-4">
        <WhatsAppCTA />
        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="grid grid-cols-3 bg-[#0c0f1e] border border-white/10 rounded-2xl overflow-hidden shadow-2xl w-full max-w-sm md:max-w-md">
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 md:py-16">
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {(['all', 'active', 'construction'] as const).map((filter) => (
            <button
              key={filter}
              ...
              className={cn(
                ...
                statusFilter === filter
                  ? filter === 'all' ? "bg-white text-black" : filter === 'active' ? "bg-emerald-500 text-white" : "bg-amber-500 text-white"
                  : "bg-white/5 text-[#6b7299] hover:text-white"
              )}
            >
              {filter === 'all' ? t.filterAll : filter === 'active' ? t.filterActive : t.filterWork}
            </button>
          ))}
        </div>
                ...
                <div key={cat.id} className="mb-20 last:mb-0">
                  ...
                  <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {catProjects.map((project) => (
                      <ProjectCard key={project.id} project={project} onOpenPreview={setSelectedProject} />
                    ))}
                  </motion.div>
                </div>
              );
      ...
      <footer className="relative pt-16 pb-20 px-4 text-center border-t border-white/10">
        ...
        <h2 className="font-display font-extrabold text-2xl text-white mb-3">👋 Sergiu Murgescu</h2>
        <p className="text-[#6b7299] text-sm leading-relaxed mb-8">{t.footerText}<br />{t.footerClick}</p>
        <div className="inline-block px-6 py-2 rounded-full bg-indigo-500/5 border border-indigo-500/20 text-indigo-300 font-semibold text-xs">✦ {t.footerPassion}</div>
      </footer>
    </main>
  );
}
