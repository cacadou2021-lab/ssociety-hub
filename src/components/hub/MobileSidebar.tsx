...
export default function MobileSidebar({ onProjectClick }: MobileSidebarProps) {
  ...
  return (
    <>
      <button onClick={toggleSidebar} className="lg:hidden fixed bottom-6 right-6 z-[60] p-4 rounded-full bg-indigo-600 text-white shadow-lg shadow-indigo-500/40 active:scale-90 transition-transform flex items-center justify-center" aria-label="Open Menu">
        <Menu size={24} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={toggleSidebar} className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm lg:hidden" />
            <motion.div
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              exit={{ x: '-100%' }}
            ...
            >
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-[#0e1225]">
                ...
                <div className="flex items-center gap-2">
                  <LanguageSwitcher />
                  <button onClick={toggleSidebar} className="p-2.5 rounded-xl bg-white/5 text-[#6b7299] hover:text-white transition-colors">
                    <X size={20} />
                  </button>
                </div>
              </div>
                ...
                {categories.filter(c => c.id !== 'all').map((cat) => {
                  ...
                  return (
                    <div key={cat.id} className="space-y-3">
                      <div className={cn("flex items-center justify-between px-3 py-2 rounded-lg border", style.bg, style.border)}>
                        <div className="flex items-center gap-2">
                          <span className="text-lg">{cat.emoji}</span>
                          <h3 className={cn("text-[11px] font-black uppercase tracking-widest", style.text)}>
                            {cat.id === 'resto' ? t.catResto : cat.id === 'ai' ? t.catAI : cat.id === 'mkt' ? t.catMkt : cat.id === 'energy' ? t.catEnergy : cat.id === 'ops' ? t.catOps : cat.id === 'biz' ? t.catBiz : cat.id === 'life' ? t.catLife : cat.id === 'security' ? t.catSecurity : cat.label}
                          </h3>
                        </div>
                        <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-white/10", style.text)}>{catProjects.length}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-2 pl-1">
                        {catProjects.map((project) => (
                          ...
                          >
                            <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">{project.emoji}</div>
                            <div className="flex-1 min-w-0">
                              <div className="font-bold text-white/90 truncate">{project.name}</div>
                            ...
                            <ChevronRight size={18} className="opacity-0 group-hover:opacity-100 transition-all text-indigo-400" />
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <div className="p-6 border-t border-white/5">
                <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10">
                  <p className="text-[10px] text-indigo-300 leading-relaxed">{t.footerClick}</p>
                </div>
              </div>
            </motion.div>
          </>
