...
export default function ProjectCard({ project, onOpenPreview }: ProjectCardProps) {
  ...
  const categoryColors: Record<string, string> = {
    ...
    security: 'before:bg-gradient-to-r before:from-red-500 before:to-red-400 hover:border-red-500/30',
  };
    ...
    >
      ...
      {project.videoUrl && (
        <div className="absolute inset-0 z-0 hidden md:block overflow-hidden rounded-2xl opacity-0 group-hover:opacity-60 transition-opacity duration-700 pointer-events-none">
          <video src={project.videoUrl} autoPlay muted loop playsInline className="w-full h-full object-cover scale-110 group-hover:scale-100 transition-transform duration-1000" />
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <h3 className="font-display font-bold text-white text-base md:text-lg mb-1.5 md:mb-2 leading-tight">{project.name}</h3>
        <p className="text-[#6b7299] text-xs md:text-sm leading-relaxed mb-5 md:mb-6 group-hover:text-white/90 transition-colors">{project.description}</p>
        ...
        <div className="flex items-center justify-between gap-2 md:gap-4 mt-auto">
          {project.status === 'under-construction' ? (
            ...
            <a
              ...
              className={cn(
                "inline-flex items-center gap-1.5 md:gap-2 px-4 py-2 md:px-5 md:py-2.5 rounded-full text-xs md:text-sm font-bold transition-all hover:opacity-90 active:scale-95 shadow-lg text-white",
