...
export default function DownloadSection() {
  ...
  const presentations = [
    { title: "Prezentare MBEuroparts (Română)", description: "Franciză Digitală AI · Piese Mercedes.", filename: "MBEuroparts-RO.pdf", lang: "RO", color: "from-amber-500 to-orange-600" },
    { title: "MBEuroparts Presentation (English)", description: "AI Digital Franchise · Mercedes Parts.", filename: "MBEuroparts-EN.pdf", lang: "EN", color: "from-indigo-500 to-blue-600" }
  ];
      ...
      <div className="bg-[#0c0f1e]/50 backdrop-blur-sm border border-white/10 rounded-[2rem] md:rounded-3xl p-8 md:p-10 overflow-hidden relative shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="text-center lg:text-left flex-1">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-6">
              <FileText size={12} />
              {t.pdfResources}
            </motion.div>
            <h2 className="text-3xl md:text-5xl font-display font-black text-white mb-6 tracking-tight leading-tight">{t.downloadPdf}</h2>
            <p className="text-[#a1a7c5] text-sm md:text-lg leading-relaxed max-w-xl mx-auto lg:mx-0">{t.pdfDescription}</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5 w-full lg:w-auto shrink-0">
            {presentations.map((pdf, idx) => (
              ...
              >
                ...
                <h3 className="text-white font-black text-base md:text-lg mb-2 group-hover:text-indigo-400 transition-colors">{pdf.title}</h3>
                <p className="text-[#6b7299] text-xs leading-relaxed mb-6 font-medium">{pdf.description}</p>
